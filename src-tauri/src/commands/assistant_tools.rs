...
pub(crate) fn assistant_tool_definitions() -> Vec<AssistantToolDefinition> {
    ...
    const TOOLS: &[ToolSpec] = &[
        ...
        ToolSpec {
            ...
            description: "Apply a pipeline of deterministic operations to a tag field. Supports regex_replace, regex_extract, strip_prefix, strip_suffix, trim, lowercase, uppercase, title_case, prettify, split_artists, chinese_to_simplified, chinese_to_traditional. Use split_artists on the plural artists field to repair every malformed single value in the requested scope without enumerating tracks. Source can be a tag field or filename stem. regex_extract writes the captured group to the destination field — group_index defaults to 1, and a pattern with no capturing group extracts nothing, so every track is skipped. The pattern must match EVERY track in scope: to group by the title before '(' or '（', extract the leading run of non-parenthesis characters (e.g. the group in '^([^(（]+)'), which also matches paren-less titles and extracts the whole title for them; the app skips a track only when the destination already holds that value. A pattern that requires '(' to be present (e.g. '^(.+)[（(]') matches nothing for paren-less titles like 红昭愿, and those tracks are skipped.",
            read_only: false, public: true,
            operation_kind: Kind::MetadataEdit,
        },
        ToolSpec {
