...
| Native shell: folder dialog / context menu / focus / quit guard | ✅ green on macOS | Folder/context/focus commands are wired with E2E overrides. On an unlocked real display, the packaged app opened and cancelled the native folder picker, opened and dismissed the native track context menu against copied media, and stayed running. Quit observes managed queued+running writes, prevents exit, coalesces duplicate requests, shows a nonblocking warning, and uses a force flag before re-requesting exit; state-machine tests cover cancel/re-prompt, coalescing, and force bypass. The macOS default predefined Quit item bypassed Tauri's exit event, so only that final default-menu item is replaced with an identical Cmd+Q item routed through `AppHandle::exit`; a live 1,000-file copied-MP3 batch proved the warning appears only after disk writes begin, Cancel keeps the app running, and the batch completes. Windows/Linux native-display validation remains CI-only. |
...
| Providers/auto-tag/audit/assistant | ✅ green | Live release-gate tests pass through the production Rust clients for MusicBrainz search/detail/DTO parsing, authenticated Discogs search/detail/DTO parsing, and credentialed OpenRouter schema-constrained JSON/usage parsing via `just fe-smoke-openrouter` and `.env.local`. Audit is wired end-to-end: deterministic checks, targeted OpenRouter review, 0.92 approval threshold, approval-only writes, two-album concurrency, specified-track grouping, cancellation/cleanup, and the Electron event/summary contract. Auto-tag is renderer-wired with real-file parsing/writes, direct and validated missing-ID artist identity, cached artist-scoped release ranking before generic MusicBrainz/Discogs search, cache merge, conditional LLM correction/genre fill, deterministic recording-ID/title/filename/duration alignment, OpenCC write conversion, suppression-aware cover resolution, local/optional remote lyrics, cancellation, polling, and terminal events. Shared OpenRouter preserves schema/auth/deadline/retry/repair behavior. Assistant has a bounded multi-step loop, typed schemas, one argument-repair attempt, repetition guard, all 21 registry tools exposed, deterministic native execution for all mutating macros, dependency-ordered plans with referenced outputs, explicit no-change evidence, and preview-only approval. Non-autonomous requests return on the first preview; autonomous requests may continue planning but never bypass approval. |
| E2E→WebdriverIO + CI + cutover | ✅ implemented; cross-platform CI evidence pending | Test-only optional Rust/renderer WebdriverIO plugins use the embedded provider and never ship in production builds. Eight native macOS tests cover visible-shell boot, folder selection through an isolated E2E override, absolute-path library read, standard/extra metadata writes and readback, assistant preview/apply, audit approval/writeback, offline auto-tag task polling, Convert UI writes, and Number UI batch writes. CI runs the same suite on macOS ARM64, macOS Intel, Windows, and Linux. Its unsigned four-runner bundle matrix verifies and launches the macOS app/DMG, silently installs and launches Windows NSIS, launches Linux AppImage, and installs and launches the Debian package. ARM64 and x64 macOS `.app`/DMG builds and checksums are green locally; the x64 Mach-O also launches under Rosetta. Windows/Linux execution remains a runner-only validation. |

> Native macOS WebdriverIO, packaged-app smoke, system folder picker, track
> context menu, and quit-during-real-write warning/Cancel flow are green on an
> unlocked display. Windows/Linux E2E and installer launch evidence comes from
> the configured CI runners rather than this Mac.
...
| `track:context-menu`      | `showTrackContextMenu`  | main.ts    | Rust shell unit tests + E2E override | wired native menu + clipboard; E2E action override, exact Copy All payload, single-popup action state. A live packaged-app check opened and dismissed the real native menu against copied media. |
...
| `cover:set`                 | `setCover`            | cover.ts  | Rust cover tests + `cover.test.ts` + native macOS smoke | wired: native image picker, max-500 JPEG quality 90 `cover.jpg`, clears suppression, returns quality-85 data URL. `just fe-smoke-cover-picker` opens the real picker and verifies cancellation returns null rather than an error. |
...
| `assistant:init-runtime`    | `assistantInitRuntime`  | assistant.ts | Rust session/schema/tool-catalog tests; production OpenRouter client and full packaged assistant loopback live smokes green | ✅ initializes idempotent UUID/session-number + configured/default compatible cache.db; exact 21-name tool catalog is characterized and exposed, with native execution or preview planning for every registered tool. `just fe-smoke-assistant` verifies renderer adapter → Tauri command → OpenRouter schema response → persisted API-call count. |
...
| `dialog:open-folder`  | `openFolderDialog` | main.ts  | E2E + real macOS display | wired; selected/null parity plus real native Open window and Escape cancellation are green; plugin GUI-error rejection remains characterized separately |
...
- [ ] WebdriverIO Tauri E2E on macOS ARM64/Intel, Windows, and Linux (macOS ARM64 green locally; remaining CI jobs pending)
- [x] copied-real-media smoke tests under `/private/tmp`
- [x] `git diff --check`
- [ ] packaged app launch + installer smoke (macOS ARM64 and x64 `.app` launch and DMG verify green; Windows/Linux CI pending)
