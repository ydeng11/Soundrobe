...
| Mutation + media-safety slice | ⏳ six formats green; WAV characterized; APE/FLAC edges/batch pending | MP3/FLAC/OGG/Opus/M4A/MP4 dispatch through one validated queue. WAV characterization on real PCM records: first ID3 addition is `full_rewrite/format_requires_full_rewrite`, identical second patch is `skipped/unchanged` byte-for-byte, null clears, and PCM `data` bytes remain exact. AIFF remains intentionally unsupported like Electron. PENDING: Rust WAV core, APE, FLAC ghost/APE/duplicate/insufficient-padding edges, and batch. |
