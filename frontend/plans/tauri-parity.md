...
| Mutation + media-safety slice | ⏳ single MP3 command + quit guard green; batch/formats pending | Atomic MP3 `track_write` runs through a managed serialized queue; queued/running work drives the native nonblocking quit confirmation with force-exit bypass. MP3 returns void; non-MP3 fails loudly without touching bytes. Tri-state/no-op/payload/readback/unknown-frame guarantees remain green. `tracks_batch_write`, progress/cancellation, and non-MP3 writers remain absent. Next: format expansion before batch exposure. |
