...
| Mutation + media-safety slice | ⏳ MP3 command green; FLAC characterized; more formats/batch pending | Atomic MP3 `track_write` + managed queue + quit guard are green. Electron FLAC characterization now uses a deterministic committed padded fixture: identical patch is `skipped/unchanged`; rich update is `metadata_rewrite/metadata_region_repacked` with fixed file size/audio offset and identical audio bytes; omitted-vs-null is covered. Non-MP3 Tauri writes still fail loudly untouched. Next: atomic FLAC core preserving metadata-region/audio invariants, then OGG/Opus. |
