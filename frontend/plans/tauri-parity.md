...
| Providers/auto-tag/audit/assistant | ⏳ partial | Deterministic audit core and approval writes are ported. Shared OpenRouter structured JSON client now preserves schema payloads, auth, total deadline, retryable/provider-error handling, cancellation, strict content validation, and one reasoning-disabled repair attempt (4 localhost Rust contracts). Audit LLM integration/run events, auto-tag, and assistant send/apply remain pending. |
