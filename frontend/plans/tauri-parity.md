...
| Mutation + media-safety slice | ⏳ pure MP3 core green; command/queue pending | Electron MP3 characterization records legacy full rewrites and payload invariants. Rust `mutations.rs` now has tri-state patches for all `DesktopAPI.writeTrack` fields, direct ID3v2 preservation, raw repeated ARTISTS recovery, intentional semantic no-op, sibling-file write/readback, exact MPEG payload equality, unknown TXXX/binary-frame survival, and platform atomic replacement (rename / `MoveFileExW`). Malformed failures leave source bytes untouched. No Tauri mutation command, managed queue, batch write, progress, or quit guard is exposed yet. Next: managed serialized `WriteQueue` + single `track_write`, then quit guard. |
