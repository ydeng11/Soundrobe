...
| Mutation + media-safety slice | ⏳ ordinary writes + batch green; FLAC repair edges pending | `track_write` supports MP3, FLAC, OGG, Opus, M4A, MP4, WAV, and APE through one queue; AIFF rejects identically. `tracks_batch_write` holds that queue for the full sequential batch, returns void, supports empty batches, and stops on first error after preserving prior commits (Electron is non-transactional). Electron exposes no batch progress/cancellation event, so none is invented. FLAC edge characterization on shared deterministic fixtures records exact outcomes: trailing APE → `full_rewrite/ape_tag_removed`; appended ghost VC → `full_rewrite/ghost_vorbis_removed` with only its length word zeroed in payload; duplicate metadata VC → `metadata_rewrite/duplicate_vorbis_removed`; 8-byte padding growth → `full_rewrite/insufficient_metadata_space`. PENDING: Rust FLAC repair edges and extra-tag/rename mutation rows. |
