...
| Mutation + media-safety slice | ⏳ MP3/FLAC/OGG/Opus green; MP4 characterized; more/batch pending | Four formats dispatch through one validated queue. MP4/M4A characterization records a severe Electron placeholder defect: write reports successful full rewrite and preserves `mdat`, but malformed parent atom sizes make both files unreadable (`End-Of-Stream`). Tauri will require valid normalized readback and exact `mdat` rather than reproduce corruption. PENDING: Rust MP4/M4A core, WAV/AIFF/APE, FLAC ghost/APE/duplicate/insufficient-padding edges, and batch. |
