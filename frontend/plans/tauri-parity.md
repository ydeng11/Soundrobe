...
| Providers/auto-tag/audit/assistant | ⏳ partial | Deterministic single-album audit core and approval-only audit writes are ported: folder/filename evidence, safe-vs-ambiguous artist splitting, genre review targets, album discovery, cancellation checks, unreadable-track containment, read-only plans, tri-state corrections, merged per-file writes, partial-failure reporting, and atomic `WriteQueue` serialization (9 Rust tests with real MP3 readback/payload checks). LLM review, run wrappers/events, auto-tag, and assistant send/apply remain pending. |
