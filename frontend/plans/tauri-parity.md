...
| Mutation + media-safety slice | ⏳ corpus ready; reader/writer pending | Shared committed Electron/Rust corpus at `test/fixtures/tauri/media-corpus`: MP3/FLAC/WAV/OGG, APEv2+trailing-ID3v1 fallback, truncated FLAC, and malformed-Vorbis-length FLAC; `expected.json` is normalized Electron `readTrackMetadata` baseline. Generator is extracted from established Electron test helpers; corpus test byte-compares generated/committed files. Next: pinned `lofty` reader DTO + differential parity. |
...
| Ch                        | Renderer method          | Owner      | Parity tests                                                            | Notes |
|---------------------------|--------------------------|------------|-------------------------------------------------------------------------|-------|
| `album:read`              | `readAlbum`              | tracks.ts  | `tracks.test.ts`                                                        | full TrackData[] readback |
