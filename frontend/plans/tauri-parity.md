...
| Providers/auto-tag/audit/assistant | ⏳ partial | Audit is wired end-to-end: deterministic checks, targeted OpenRouter review, 0.92 approval threshold, approval-only writes, two-album concurrency, specified-track grouping, cancellation/cleanup, and the Electron event/summary contract. Auto-tag is renderer-wired with real-file parsing/writes, exact provider IDs, generic MusicBrainz/Discogs lookup, cache merge, conditional LLM correction/genre fill, cancellation, polling, and terminal events; artist-scoped release ranking, track-protection, covers/lyrics, and Chinese write conversion remain pending. Shared OpenRouter preserves schema/auth/deadline/retry/repair behavior. Assistant action approval is wired; assistant send remains pending. |
