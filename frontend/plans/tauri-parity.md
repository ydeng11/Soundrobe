...
| Providers/auto-tag/audit/assistant | ⏳ partial | Audit is wired end-to-end: deterministic checks, targeted OpenRouter review, 0.92 approval threshold, approval-only writes, two-album concurrency, specified-track grouping, cancellation/cleanup, and the Electron event/summary contract. Auto-tag is renderer-wired with real-file parsing/writes, direct and validated missing-ID artist identity, cached artist-scoped release ranking before generic MusicBrainz/Discogs search, cache merge, conditional LLM correction/genre fill, deterministic recording-ID/title/filename/duration alignment, OpenCC write conversion, suppression-aware cover resolution, local/optional remote lyrics, cancellation, polling, and terminal events. Shared OpenRouter preserves schema/auth/deadline/retry/repair behavior. Assistant now has a bounded multi-step loop, typed schemas, one argument-repair attempt, repetition guard, ten executable read-only tools, deterministic auto-tag/audit/track-numbering/title-prefix/filename-prefix/filename-inference/Chinese-conversion/album-grouping routes, and preview-only approval. Five mutating registry macros remain unimplemented. |
| E2E→WebdriverIO + CI + cutover | ⏳ runtime validation pending | Renderer/Rust gates and an unsigned four-runner bundle matrix are defined. WebdriverIO GUI coverage, cross-platform installer launch, and real-display native-shell timing remain environment-blocked. |
...
| Ch                        | Renderer method          | Owner        | Parity tests                                                                 | Notes |
|---------------------------|--------------------------|--------------|------------------------------------------------------------------------------|-------|
| `assistant:send`           | `assistantSend`         | assistant.ts | Rust scope/preview/tool-schema/OpenRouter contracts; loopback integration pending | 🟡 wired: real configured credentials, persisted user/API/tool/assistant entries, cancellation, exact step/tool events, bounded ten-step loop, structured tool evidence, typed argument validation with one repair attempt, repeated-call stop, and preview batches constrained to active paths, supported kinds, tag kinds, risk levels, and fields. Ten read-only tools execute natively; auto-tag, audit, album/disc track numbering, title/filename prefix cleanup, filename tag inference, bidirectional Chinese conversion, and collision-safe album grouping route deterministically with explicit no-change or clarification. Five remaining mutating registry macros and autonomous apply remain pending. |
