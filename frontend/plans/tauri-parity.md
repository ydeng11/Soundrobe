...
| Providers/auto-tag/audit/assistant | ⏳ partial | Deterministic audit core and approval writes are ported. Shared OpenRouter structured JSON client preserves schema payloads, auth, total deadline, retryable/provider-error handling, cancellation, strict content validation, and one reasoning-disabled repair attempt. Audit semantic review now has the Electron prompt/schema, targeted single-field normalization, 0.92 approval threshold, deterministic merge precedence, and explicit no-client warnings. Discogs alias preflight and run commands/events remain pending, so renderer audit entry points are still intentionally unwired. Auto-tag and assistant send/apply also remain pending. |
...
| Ch                       | Renderer method      | Owner    | Parity tests                                          | Notes |
|--------------------------|----------------------|----------|-------------------------------------------------------|-------|
| `audit:run`              | `runAudit`          | audit.ts | `audit.test.ts`, `AuditRuleEngine.test.ts`            | deterministic discovery/core ported; wrapper/events and configured LLM review pending |
| `audit:run-specified`     | `runAuditOnTracks` / `runAuditOnAlbums` | audit.ts | `audit.test.ts` | grouping DTO/core ported; wrapper/events and configured LLM review pending |
| `audit:run-album`         | `runAlbumAudit`     | audit.ts | `audit.test.ts`                                      | deterministic read-only album core ported; configured LLM review and command wiring pending |
