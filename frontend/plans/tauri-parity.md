...
| Ch              | Renderer method    | Owner    | Parity tests        | Notes |
|-----------------|--------------------|----------|---------------------|-------|
| `album:auto-tag` | `autoTagAlbum`    | main.ts  | Rust auto-tag contracts + real copied-MP3 write/readback; frontend suite pending cutover | 🟡 wired: validates directory, returns taskId, runs real-file parse→cache/direct-ID→cached artist releases→MusicBrainz/Discogs fallback→conditional LLM→merge→genre→OpenCC conversion→safe write→cover/lyrics in background, with polling/cancellation and terminal events. Full RemoteTrackMatcher protection, canonical artist identity completion, and exact source/merge/write event detail remain pending. |
