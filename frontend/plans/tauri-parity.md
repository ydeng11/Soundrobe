...
| Ch              | Renderer method    | Owner    | Parity tests        | Notes |
|-----------------|--------------------|----------|---------------------|-------|
| `album:auto-tag` | `autoTagAlbum`    | main.ts  | Rust auto-tag contracts + real copied-MP3 write/readback; frontend suite pending cutover | 🟡 wired: validates directory, returns taskId, runs real-file parse→cache/direct-ID→cached artist releases→MusicBrainz/Discogs fallback→conditional LLM→merge→recording-ID/title/filename/duration alignment→genre→OpenCC conversion→safe write→cover/lyrics in background, with polling/cancellation and terminal events. Release-detail ranking now prefers one-to-one title evidence and rejects duration mismatches; missing-ID artist discovery and exact source/merge/write event detail remain pending. Partial media-write failures now fail the task after all files are attempted. |
