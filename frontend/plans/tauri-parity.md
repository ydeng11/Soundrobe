...
| Mutation + media-safety slice | ⏳ all ordinary single-track formats green; FLAC edges/batch pending | `track_write` supports MP3, FLAC, OGG, Opus, M4A, MP4, WAV, and APE through one queue; AIFF rejects with Electron’s exact message. APE computes MAC descriptor audio end, strips old APEv2+ID3v1 in the sibling, writes canonical multi-value/provider APEv2, requires exact audio core + valid readback, supports no-op/null/unknown/malformed guarantees, then atomically replaces. PENDING: FLAC ghost/APE/duplicate/insufficient-padding edges and `tracks_batch_write` progress/cancellation. |
