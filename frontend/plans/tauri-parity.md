...
| Mutation + media-safety slice | ⏳ MP3 + padded FLAC green; OGG/Opus characterized; more/batch pending | MP3/FLAC dispatch through one queue with atomic validation. OGG characterization proves full container rewrite preserves encoded audio packets; identical patch still rewrites. True Opus exposes an Electron defect: handler returns success/full-rewrite but does not recognize `OpusTags`, leaving bytes/tags unchanged. Tauri will implement true Opus per migration plan rather than reproduce silent failure. PENDING: Rust OGG/Opus core; FLAC ghost/APE/duplicate/insufficient-padding edges; other formats and batch. |
