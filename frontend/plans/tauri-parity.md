...
| E2E→WebdriverIO + CI + cutover | ✅ implemented; cross-platform CI evidence pending | Test-only optional Rust/renderer WebdriverIO plugins use the embedded provider and never ship in production builds. The native macOS smoke launches the release binary, waits for the visible renderer, and invokes `app_info`; it is green locally. CI runs the same smoke on macOS, Windows, and Linux, alongside the unsigned four-runner bundle matrix. macOS `.app` launch, CI-safe DMG creation, and DMG checksum verification are green locally; Windows/Linux execution remains a runner-only validation. |
