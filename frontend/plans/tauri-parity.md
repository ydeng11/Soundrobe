...
| Providers/auto-tag/audit/assistant | ⏳ partial | Deterministic single-album audit core is ported: folder/filename evidence, safe-vs-ambiguous artist splitting, genre review targets, flat/direct/nested album discovery, cancellation checks, unreadable-track containment, and read-only fix plans (7 Rust tests, including real tagged-MP3 byte preservation). LLM review, run wrappers/events, approval writes, auto-tag, and assistant send/apply remain pending. |
