...
| Read-only library slice — traversal/grouping/read core | ⏳ partial | `library_scan` grouping has a shared Electron/Rust fixture. Internal `TrackData` reader now uses pinned Lofty 0.24 plus bounded MPEG/OGG/FLAC/APE fallbacks; it differentially matches all seven committed Electron corpus payloads (normal MP3/FLAC/WAV/OGG, APE+ID3v1, malformed FLAC; finite durations normalized to 12 decimals for one Node/Rust ULP). `directory_list` byte-order still diverges from Electron `localeCompare` for case/diacritics/CJK. PENDING exposed reads: `directory:read`, `album:read`, `album:refresh`; generic native Discogs IDs/cover discovery remain later slices. |
| Mutation + media-safety slice | ⏳ reader core green; writer pending | Shared committed Electron/Rust corpus at `test/fixtures/tauri/media-corpus` is consumed by the internal Lofty/custom-fallback `TrackData` reader. No writer, queue, or media mutation is implemented yet. Next: expose read commands, then format-by-format no-op/MP3 payload-hash writer parity. |
...
| `album:refresh`        | `refreshAlbum`    | library.ts       | `library.test.ts`                         | re-read a single album (internal TrackData reader ready; album command pending) |
...
| `directory:read`       | `readDirectory`   | directory.ts     | `directory.test.ts`                       | subdir + track readback + audioCount (TrackData reader ready; command pending) |

### Tracks (metadata read/write/rename/extra-tags/delete)

| Ch                        | Renderer method          | Owner      | Parity tests                                                            | Notes |
|---------------------------|--------------------------|------------|-------------------------------------------------------------------------|-------|
| `album:read`              | `readAlbum`              | tracks.ts  | `tracks.test.ts`                                                        | internal TrackData reader ported/corpus-verified; album DTO + cover discovery pending |
