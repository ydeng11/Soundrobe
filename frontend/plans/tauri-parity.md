...
| Read-only library slice — traversal/grouping/read core | ⏳ partial | `library_scan` grouping has a shared Electron/Rust fixture. Internal `TrackData` reader uses pinned Lofty 0.24 plus bounded MPEG/OGG/FLAC/APE fallbacks; it differentially matches all seven corpus payloads (finite durations normalized to 12 decimals for one Node/Rust ULP). `directory:read` is wired with Electron-equivalent per-file unreadable fallback (real size + basename) and normal OGG integration coverage. `directory_list` byte-order still diverges from Electron `localeCompare` for case/diacritics/CJK. PENDING exposed reads: `album:read`, `album:refresh`; generic native Discogs IDs/cover discovery remain later slices. |
