...
| Mutation + media-safety slice | ⏳ single MP3 command green; quit guard/batch/formats pending | Pure atomic MP3 core is wired as `track_write` through one managed async `WriteQueue`; blocking mutation starts only after acquiring the gate, and queued/running work counts active. MP3 returns Electron-compatible void; non-MP3 fails loudly as pending without touching bytes. Tri-state/no-op/payload/readback/unknown-frame guarantees remain green. `tracks_batch_write`, progress/cancellation, non-MP3 writers, and quit-during-write guard are not exposed yet. Next: quit guard using queue activity, then format expansion before batch. |
