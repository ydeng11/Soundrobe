...
| Mutation + media-safety slice | ⏳ MP3/FLAC/OGG/Opus/M4A/MP4 green; more/batch pending | Six formats dispatch through one validated queue. M4A/MP4 use native ilst rewriting, tri-state standard/freeform fields, exact `mdat` equality, valid normalized readback, no-op detection, unknown-atom survival, and atomic replacement—intentionally fixing Electron’s characterized corrupt placeholder writes. Native multi-value ARTISTS/ALBUMARTISTS and Discogs readback are wired. PENDING: WAV/AIFF/APE writers, FLAC ghost/APE/duplicate/insufficient-padding edges, and batch. |
