...
| Mutation + media-safety slice | ⏳ seven writable formats + AIFF error green; APE/FLAC edges/batch pending | MP3/FLAC/OGG/Opus/M4A/MP4/WAV dispatch through one queue. WAV reuses tri-state ID3 mapping, exact RIFF `data` equality, valid readback, no-op/null/provider/malformed guarantees, and atomic replacement. AIFF returns Electron’s exact unsupported message. PENDING: APE writer, FLAC ghost/APE/duplicate/insufficient-padding edges, and batch. |
