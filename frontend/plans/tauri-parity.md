...
| Providers/auto-tag/audit/assistant | ⏳ partial | Audit is wired end-to-end: deterministic checks, targeted OpenRouter review, 0.92 approval threshold, approval-only writes, two-album concurrency, specified-track grouping, cancellation/cleanup, and the Electron event/summary contract. Auto-tag is renderer-wired with real-file parsing/writes, exact provider IDs, generic MusicBrainz/Discogs lookup, cache merge, conditional LLM correction/genre fill, cancellation, polling, and terminal events; artist-scoped release ranking, track-protection, covers/lyrics, and Chinese write conversion remain pending. Shared OpenRouter preserves schema/auth/deadline/retry/repair behavior. Assistant send and approval are wired preview-first; the Electron multi-step tool registry and deterministic macro routing remain pending. |
...
| Ch              | Renderer method    | Owner    | Parity tests        | Notes |
|-----------------|--------------------|----------|---------------------|-------|
| `album:auto-tag` | `autoTagAlbum`    | main.ts  | Rust auto-tag contracts + real copied-MP3 write/readback; frontend suite pending cutover | 🟡 wired: validates directory, returns taskId, runs real-file parse→cache/direct-ID→MusicBrainz→Discogs→conditional LLM→merge→genre→safe write in background, with polling/cancellation and progress/completed/failed/cancelled events. Artist-scoped release ranking, track-protection, cover/lyrics, Chinese write conversion, and exact source/merge/write event detail remain pending. |
