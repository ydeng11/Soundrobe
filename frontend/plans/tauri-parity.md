...
| Providers/auto-tag/audit/assistant | ⏳ partial | Audit is wired end-to-end: deterministic checks, targeted OpenRouter review, 0.92 approval threshold, approval-only writes, two-album concurrency, specified-track grouping, cancellation/cleanup, and the Electron event/summary contract. Auto-tag is renderer-wired with real-file parsing/writes, exact provider IDs, cached artist-scoped release ranking before generic MusicBrainz/Discogs search, cache merge, conditional LLM correction/genre fill, conservative provider-track protection, OpenCC write conversion, suppression-aware cover resolution, local/optional remote lyrics, cancellation, polling, and terminal events; the Electron matcher's duration/filename scoring and missing-ID artist identity discovery remain pending. Shared OpenRouter preserves schema/auth/deadline/retry/repair behavior. Assistant send and approval are wired preview-first; the Electron multi-step tool registry and deterministic macro routing remain pending. |
...
| Ch              | Renderer method    | Owner    | Parity tests        | Notes |
|-----------------|--------------------|----------|---------------------|-------|
| `album:auto-tag` | `autoTagAlbum`    | main.ts  | Rust auto-tag contracts + real copied-MP3 write/readback; frontend suite pending cutover | 🟡 wired: validates directory, returns taskId, runs real-file parse→cache/direct-ID→cached artist releases→MusicBrainz/Discogs fallback→conditional LLM→merge→provider-track protection→genre→OpenCC conversion→safe write→cover/lyrics in background, with polling/cancellation and terminal events. Electron duration/filename match scoring, missing-ID artist discovery, and exact source/merge/write event detail remain pending. |
...
| Ch                        | Renderer method          | Owner        | Parity tests                                                                 | Notes |
|---------------------------|--------------------------|--------------|------------------------------------------------------------------------------|-------|
| `assistant:send`           | `assistantSend`         | assistant.ts | Rust scope/preview/OpenRouter contracts; Electron tool-loop suite pending | 🟡 wired: real configured credentials, persisted user/API/assistant entries, cancellation, message/error events, and preview batches constrained to active paths, supported kinds, tag kinds, risk levels, and fields. Electron's multi-step read-only tool loop, deterministic macro routing, argument repair, and autonomous apply remain pending. |
