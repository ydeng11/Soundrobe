...
| Read-only library slice — traversal/grouping/read core | ⏳ partial | `library_scan` has a shared fixture. Internal `TrackData` reader uses Lofty 0.24 plus bounded MPEG/OGG/FLAC/APE fallbacks and matches the seven-file corpus, including rich MP3 ID3: repeated artists, totals, MB/Discogs TXXX, description, canonical lyrics text, and embedded artwork (finite durations normalized to 12 decimals for one Node/Rust ULP). `directory:read`, `album:read`, `album:refresh` are wired with per-file fallback, hints/status, and local cover state. PENDING: `directory_list` ICU collation, cover data URLs/provider discovery, and M4A/MP4/Opus/AIFF corpus parity. |
| Mutation + media-safety slice | ⏳ reader core green; writer pending | Shared corpus now includes a production-written rich ID3 MP3 (standard fields, repeated artists, totals, MB/Discogs IDs, comments/description/lyrics, artwork) plus FLAC/WAV/OGG/APE/malformed cases. Rust Lofty+fallback reader matches normalized DTOs. Raw Electron ID3 lyrics object leakage is characterized; normalized baseline follows canonical `string|null` by extracting `.text`. MP3 compilation write→read `null` is separately characterized. No Rust writer, queue, or mutation yet. Next: remaining read containers, then no-op/MP3 payload-hash writer parity. |
