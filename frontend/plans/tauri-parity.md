...
| Mutation + media-safety slice | ⏳ MP3 + padded FLAC commands green; ghost/other formats/batch pending | `track_write` now dispatches MP3 and FLAC through one queue. FLAC uses format-specific VorbisComments, atomic sibling validation, exact audio payload equality, and a bounded metadata repacker that retains original padded size/audio offset; no-op/null/rich/provider/unknown-comment/malformed cases are green. Native FLAC Discogs readback is wired. PENDING before full FLAC parity: ghost Vorbis blocks inside audio, prepended/trailing APE removal, duplicate-comment repair, and insufficient-padding full-rewrite characterization. OGG/Opus and batch remain absent. |
