...
| Mutation + media-safety slice | ⏳ MP3/FLAC/OGG/Opus commands green; more formats/FLAC edges/batch pending | `track_write` dispatches MP3, padded FLAC, OGG/Vorbis, and true Opus through one queue. OGG/Opus writes use format-specific comments, atomic sibling validation, and exact logical encoded-audio packet equality; semantic no-op avoids Electron’s unnecessary OGG rewrite. True Opus tags now update correctly (intentional fix for characterized Electron silent-success defect). Native provider readback and canonical alias replacement are wired. PENDING: FLAC ghost/APE/duplicate/insufficient-padding edges, MP4/M4A/WAV/AIFF/APE writers, and batch. |
