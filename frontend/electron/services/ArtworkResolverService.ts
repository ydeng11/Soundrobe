...
async function findDiscogs(
  ...
  if (!artist || !album) {
    logger.debug("cover", "findDiscogs: skip — no artist or album name");
    return null;
  }
  ...
  try {
    // Search releases with per_page=10 for candidate filtering
    const searchUrl = `${DISCOGS_BASE}/database/search?type=release&per_page=10&q=${encodeURIComponent(`${artist} ${album}`)}`;
    const headers: Record<string, string> = {
      "User-Agent": "auto-tagger/0.1.0",
    };
    if (token) headers["Authorization"] = `Discogs token=${token}`;

    logger.debug("cover", `findDiscogs: searching q="${artist} ${album}" per_page=10 token=${!!token}`);

    const searchRes = await fetch(searchUrl, { headers, signal: AbortSignal.timeout(10_000) });
    if (!searchRes.ok) {
      logger.debug("cover", `findDiscogs: search HTTP ${searchRes.status}`);
      return null;
    }

    const searchData = (await searchRes.json()) as {
      results?: Array<{ cover_image?: string; title?: string; id?: number }>;
    };
    const results = searchData.results ?? [];
    logger.debug("cover", `findDiscogs: search returned ${results.length} results`);

    if (results.length === 0) return null;

    // Scan all candidates for a valid artist+album match
    for (let i = 0; i < results.length; i++) {
      const candidate = results[i];
      const title = candidate.title ?? "";

      // Parse Discogs title as "Artist - Album"
      ...
      const artistOk = await artistMatchesQuery(parsed.artist, ctx.artistName!);
      if (!artistOk) {
      ...
      const albumOk = await albumMatchesQuery(parsed.album, ctx.albumName!, ctx.artistName!);
      if (!albumOk) {
        logger.debug("cover", `findDiscogs: candidate[${i}] REJECT album — title="${title}"`);
        continue;
      }

      // Found a match — use this candidate's cover
      const coverUrl = candidate.cover_image;
      if (!coverUrl) {
        logger.debug("cover", `findDiscogs: candidate[${i}] ACCEPT artist+album but has no cover_image — title="${title}" id=${candidate.id ?? "?"}`);
        continue;
      }

      logger.info("cover", `findDiscogs: candidate[${i}] ACCEPTED title="${title}" id=${candidate.id ?? "?"} url=${coverUrl}`);

      // Download the cover image
      const imgRes = await fetch(coverUrl, { signal: AbortSignal.timeout(10_000) });
      if (!imgRes.ok) {
        logger.debug("cover", `findDiscogs: image fetch HTTP ${imgRes.status} — try next candidate`);
        continue;
      }

      const bytes = Buffer.from(await imgRes.arrayBuffer());
      const mime = imgRes.headers.get("content-type") ?? "image/jpeg";
      logger.debug("cover", `findDiscogs: downloaded ${bytes.length} bytes, type=${mime}`);

      return { kind: ctx.kind, source: "discogs", bytes, mime, url: coverUrl };
    }

    logger.info("cover", `findDiscogs: no acceptable candidate among ${results.length} results for q="${artist} ${album}"`);
    return null;
  } catch (err) {
