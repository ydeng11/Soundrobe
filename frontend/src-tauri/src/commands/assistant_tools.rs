...
pub(crate) fn assistant_tool_definitions() -> Vec<AssistantToolDefinition> {
    ...
    const TOOLS: &[ToolSpec] = &[
        ...
        ToolSpec {
            ...
            description: "Apply a pipeline of deterministic operations to a tag field. Supports regex_replace, regex_extract, strip_prefix, strip_suffix, trim, lowercase, uppercase, title_case, prettify, split_artists, chinese_to_simplified, chinese_to_traditional. Use split_artists on the plural artists field to repair every malformed single value in the requested scope without enumerating tracks. Source can be a tag field or filename stem. regex_extract requires the pattern to contain a capturing group — group_index defaults to 1, and a pattern without one (e.g. one that only matches the whole string) extracts nothing and every track is skipped. The pattern must also match EVERY track in scope: to group by the title before '(' or '（', extract the leading run of non-parenthesis characters (e.g. the group that matches one or more chars from the start). A pattern that requires '(' to be present matches nothing for paren-less titles like 红昭愿, and those tracks are skipped.",
            read_only: false, public: true,
            operation_kind: Kind::MetadataEdit,
        },
        ToolSpec {
