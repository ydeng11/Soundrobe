...
- **Global scope**: `/Users/ihelio/code/auto_tagger/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/ihelio/code/auto_tagger/.claude/notes/`)
...
2. Glob `/Users/ihelio/code/auto_tagger/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/ihelio/code/auto_tagger/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/ihelio/code/auto_tagger/.claude/notes/` — works in any directory
