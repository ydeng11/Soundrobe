...
- **Web-service container profile** — a non-root multi-stage image, hardened
  Compose deployment, password-file secret, persistent `/config` mount, direct
  per-library bind-mount guidance, and a container build that resolves the
  locked dependency set are available for self-hosting.

- **Web-service image publication** — tagged releases and explicit manual
...
- **Web-service auto-tagging** — authenticated browser sessions can now start
  ...
  when late work completes or fails, and cannot rewrite an already finished
  task.
...
- **Web-service runtime foundation** — browser deployments can now identify the
  ...
  release search/resolve commands with bounded provider requests, release-match
  previews, selected candidate writes, and provider-backed album/artist artwork
  downloads, organizer album sorting, and lyrics fetch/download commands are
  also available; the headless server serves the built browser SPA
